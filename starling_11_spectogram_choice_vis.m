% choice based spectrogram visualization
% One PDF per participant/page
% rows = brain areas, with left/right combined
% columns = arrowup / arrowdown
% each row has its own colorbar shared between arrowup and arrowdown
...
output_folder = fullfile('\\155.100.91.44\d\Code\Nill\Starling_neural_data\starling_11_spectogram_choice_vis\');
...
for p = 1:numel(files)
    ...
    for pageNum = 1:nPages
        ...
        title(t, sprintf('%s | Choice Spectrogram by Brain Area | Page %d', ptID, pageNum), ...
            'FontWeight', 'bold', ...
            'Interpreter', 'none');
        ...
        exportgraphics(f, ...
            fullfile(output_folder, sprintf('%s_arrowup_arrowdown_spectrogram_page_%02d.pdf', ptID, pageNum)), ...
            'ContentType', 'vector', ...
