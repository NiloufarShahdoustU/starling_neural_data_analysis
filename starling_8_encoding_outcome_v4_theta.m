...
% freqBand = [4 7]; for checking if they are encoding outcome.
clc;
clear;
close all;
...
%% Main folder
output_folder = fullfile('\\155.100.91.44\d\Code\Nill\Starling_neural_data\starling_8_encoding_outcome_v4\');
...
for p = 1:numel(ptIDs)
    ...
    %% extract mean power in high-frequency band (75–110 Hz)
    freqBand = [4 7];
    ...
    for ch = 1:nChans
        ...
        T = size(Spec,2);
        prefWindow = max(1, choiceFeedbackWindow):min(T, choiceFeedbackWindow+600-1);
