...
output_folder = fullfile('\\155.100.91.44\d\Code\Nill\Starling_neural_data\starling_12_CBPT_outcome_v2\');
...
for p = 1:numel(files)
    ...
    for a = 1:numel(brainAreas)
        ...
        sgtitle(sprintf('%s | %s | time-frequency outcome spectrogram CBPT', ...
            ptID, areaName), ...
            'FontWeight','bold', ...
            'Interpreter','none');
        ...
        pdfName = fullfile(output_folder, ...
            sprintf('%s_%s_win_lose_ttest_CBPT_spectrogram.pdf', ...
            ptID, safeAreaName));
