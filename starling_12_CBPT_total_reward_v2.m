...
output_folder = fullfile('\\155.100.91.44\d\Code\Nill\Starling_neural_data\starling_12_CBPT_total_reward_v2\');
...
for p = 1:numel(files)
    ...
    for a = 1:numel(brainAreas)
        ...
        matName = fullfile(output_folder, ...
            sprintf('%s_%s_ttest_CBPT_results.mat', ...
            ptID, safeAreaName));
        ...
        sgtitle(sprintf('%s | %s | time-frequency total reward spectrogram CBPT', ...
            ptID, areaName), ...
            'FontWeight','bold', ...
            'Interpreter','none');
        ...
        pdfName = fullfile(output_folder, ...
            sprintf('%s_%s_win_lose_total_reward_ttest_CBPT_spectrogram.pdf', ...
            ptID, safeAreaName));
...
function [sigMask, ...
    ...
    for ci = 1:numel(realStatsPos)
        ...
        if isSig
            ...
            clusterRows(end+1, :) = { ...
                "positive_win_greater_than_lose", ...
                ci, ...
                realStatsPos(ci), ...
                clusterThresholdPos, ...
                numel(pix), ...
    ...
    for ci = 1:numel(realStatsNeg)
        ...
        if isSig
            ...
            clusterRows(end+1, :) = { ...
                "negative_lose_greater_than_win", ...
                ci, ...
                realStatsNeg(ci), ...
                clusterThresholdNeg, ...
                numel(pix), ...
...
function [tMap, pMap] = compute_ttest_maps_fast( ...
    ...
    nWin = sum(~isnan(winData), 3);
    nLose = sum(~isnan(loseData), 3);

    meanWin = mean(winData, 3, 'omitnan');
    meanLose = mean(loseData, 3, 'omitnan');

    varWin = var(winData, 0, 3, 'omitnan');
    varLose = var(loseData, 0, 3, 'omitnan');

    se = sqrt((varWin ./ nWin) + (varLose ./ nLose));

    tMap = (meanWin - meanLose) ./ se;

    dfNumer = ((varWin ./ nWin) + (varLose ./ nLose)).^2;
    dfDenom = ((varWin ./ nWin).^2 ./ (nWin - 1)) + ((varLose ./ nLose).^2 ./ (nLose - 1));

    df = dfNumer ./ dfDenom;

    pMap = 2 .* tcdf(-abs(tMap), df);

    bad = nWin < 2 | nLose < 2 | se == 0 | isnan(se) | isnan(df) | df <= 0;
